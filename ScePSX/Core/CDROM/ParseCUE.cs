...
namespace ScePSX.CdRom
{
    public partial class CdDisk
    {
            ...
            foreach (var fileBlock in fileBlocks)
            {
                ...
                for (int i = 0; i < trackMatches.Count; i++)
                {
                    ...
                    CdTrack track = new CdTrack(filePath, isAudio, trackNumber, FrameToMSF(absoluteFrame))
                    {
                        ...
                        IncludePregapLength = (lengthInFrames * fileSectorSize) + (pregapFrames * fileSectorSize),
                        IncludePregapStart = absoluteFrame * fileSectorSize
                    }
            ;
        ...
        private (uint Lba, uint Size) FindSystemCnfMetadata(FileStream fs)
        {
