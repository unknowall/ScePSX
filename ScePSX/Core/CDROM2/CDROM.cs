...
namespace ScePSX.CdRom2
{
    ...
    public class CDROM
    {
        ...
        private CDSector currentSector = new CDSector(CDSector.RAW_BUFFER);
        private CDSector lastReadSector = new CDSector(CDSector.RAW_BUFFER);
            ...
            switch (mode)
            {
                ...
                case Mode.Play:
                    ...
                    if (cdDebug)
                    {
                        ...
                        Console.WriteLine($"[CDROM] Reading readLoc: {readLoc - 1} seekLoc: {seekLoc} size: {readSector.Length}");
                        Console.ResetColor();
                    }
        ...
        private void ExecuteCommand(uint value)
        {
            ...
            switch (value)
            {
                ...
                case 0x1F:
                    videoCD();
                    break;
                case uint _ when value >= 0x50 && value <= 0x57:
                    lockUnlock();
                    break;
                default:
        ...
        private void readTOC()
        {
            if (isLidOpen)
            {
                responseBuffer.EnqueueRange(stackalloc byte[] {
                    0x11,
                    0x80
                });
        ...
        private void play()
        {
            ...
            Console.WriteLine($"[CDROM] CDDA Play Track: {track} Loc: {readLoc}");
        ...
        private void getTD()
        {
            ...
            { //returns CD LBA / End of last track
                ...
                if (cdDebug)
                    Console.WriteLine($"[CDROM] Track: {track} STAT: {STAT:x2} {mm}:{ss}");
            } else
            { //returns Track Start
        ...
        private void getID()
        {
            ...
            Span<byte> response = stackalloc byte[] {
                ...
                0x45,
                0x41
            }; //SCE | //A 0x41 (America) - I 0x49 (Japan) - E 0x45 (Europe)
            responseBuffer.EnqueueRange(response);
            interruptQueue.Enqueue(0x2);
        }
