...
namespace ScePSX.UI
{
    public class RomList : ListBox
    {
        ...
        public RomList()
        {
            ...
            var item0 = new ToolStripMenuItem(Translations.GetText("RomList_RomList_mcr"), null, OnMcrClick);
            var item1 = new ToolStripMenuItem(Translations.GetText("RomList_RomList_set"), null, OnSetClick);
            var item2 = new ToolStripMenuItem(Translations.GetText("RomList_RomList_edit"), null, OnCheatClick);

            var item5 = new ToolStripMenuItem(Translations.GetText("RomList_RomList_geticon"), null, OnUpIconClick);
            var item3 = new ToolStripMenuItem(Translations.GetText("RomList_RomList_seticon"), null, OnSetIconClick);
            var item4 = new ToolStripMenuItem(Translations.GetText("RomList_RomList_del"), null, OnDeleteClick);
                    ...
                    if (_shouldSearchSubdirectories == null)
                    {
                        ...
                        this.Invoke((MethodInvoker)delegate
                        {
                            var dialogResult = MessageBox.Show(
                                Translations.GetText("RomList_SearchDir_SubDir"),
                                "",
        ...
        private void OnUpIconClick(object sender, EventArgs e)
        {
            ...
            foreach (var Slot in mcr.Slots)
            {
                ...
                if (Slot.IconFrames > 0)
                {
                    ...
                    selectedGame.Icon = GetIconBitmap(Slot, 0);
        ...
        private void DrawItems(DrawItemEventArgs e)
        {
            ...
            int titleX = bounds.Left + iconSize + 15;
            int titleY = bounds.Top + 32;

            int infoY = titleY + 15;
            DrawText(e.Graphics, $"{game.ID}", new Font(Font_Name, 9), Color.FromArgb(200, 200, 200), titleX + 15, infoY);

            string infoText = $"{Translations.GetText("RomList_DrawInfoBoxes_last")}: {game.LastPlayed}   " +
                $"{Translations.GetText("RomList_DrawInfoBoxes_statesave")}: {(game.HasSaveState ? "✓" : "✗")}  " +
                $"{Translations.GetText("RomList_DrawInfoBoxes_cheat")}: {(game.HasCheats ? "✓" : "✗")}  ";
        ...
        private void DrawIcon(Graphics g, Image icon, Rectangle bounds, int iconSize, int padding)
        {
            ...
            if (icon != null)
            {
                g.DrawImage(icon, bounds.Left + padding + 5, icony, iconSize, iconSize);
            }
        }
        ...
        private void DrawName(Graphics g, string name, Rectangle bounds, int iconSize, int padding)
        {
            ...
            using (var brush = new SolidBrush(Color.White))
            {
                ...
                g.DrawString(name, nameFont, brush, bounds.Left + iconSize + padding * 2 + 15, icony + 3);
            }
        }
        ...
        private void DrawInfoBoxes(Graphics g, Game game, Rectangle bounds, int iconSize, int padding)
        {
            ...
            if (game.LastPlayed != "" && this.Width > 550)
                DrawInfoBox(g, $"{Translations.GetText("RomList_DrawInfoBoxes_last")}: {game.LastPlayed}", startX - 29, startY, 9);
            DrawInfoBox(g, $"{Translations.GetText("RomList_DrawInfoBoxes_statesave")}: {(game.HasSaveState ? "✓" : "✗")}", startX + 166, startY, 9);
            DrawInfoBox(g, $"{Translations.GetText("RomList_DrawInfoBoxes_cheat")}: {(game.HasCheats ? "✓" : "✗")}", startX + 260, startY, 9);
        }
